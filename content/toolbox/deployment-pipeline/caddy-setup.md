...
draft: false
---
![Caddy Logo](../images/caddy-logo.png)
{.fourth}
...
1. You will need to have a Docker Image on Docker Hub with your Javalin application. If not, then follow the [Actions and Docker Hub tutorial](../actions-dockerhub) tutorial first.
2. You should also have a Javalin application running on your Droplet in a Docker Container. If that is not the case, then follow the [Hotel API tutorial](../hotelAPI-setup) tutorial first.
...
![Caddy Setup](../images/caddy-setup.png)

## Next steps

As a last step we will add Watchtower to the Docker Compose file to ensure that the containers are always up-to-date. Follow the [Watchtower Setup](../watchtower) tutorial to complete the deployment pipeline setup.
