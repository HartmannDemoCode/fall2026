...
In this part, we will create a simple REACT-SPA, that will use a backend as we created in the first part of the semester. The application must include two views, one for unauthenticated users (the login view below) and one,  rendered after a successful login.
...
![Mockup](../images/week4-ex-security.png)
...
This exercise requires your backend to run, and to handle CORS as explained in a previous session. If you are not sure how to do that, then check this [CORS and Javalin tutorial](/toolbox/security/cors). Your backend API should have the security package installed. 

1. Create a new react project and clean it up "[the usual way](/toolbox/react/vite)".

2. Create a file called `apiFacade.js` in the `/src` folder.

3. Add the following module functions:
  - login(credentials)
    - fetch data from the login endpoint on the server, and store the token received in localStorage
  - logout()
    - remove the token from localStorage
  - getTokenData()
    - get the token from localStorage, decode it and return the data contained in the token, like username, roles, expiration time etc.
  - fetchData(url, options)
    - make a request to a protected endpoint on the server, and return the data received from the server. Make sure to add the token to the request header.
    - the options object contains 3 parts:
     - the method (GET, POST etc.)
     - a boolean value indicating if the token should be added to the reques
     - a body object, if the request should have a body (POST, PUT etc.)
