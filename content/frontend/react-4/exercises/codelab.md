...
1. Create a new [React application with Vite](/toolbox/react/vite).
2. Install the react router.
3. Create 2 new folders called `components` and `pages` in the `src` folder.
4. Add the [json Mock server](/toolbox/react/json-server).
...
**Note:** You are expected to apply the following **React Context** and **CSS Modules for theming** from the light/dark theme tutorial. See tutorial [here](/toolbox/react/css-theming).
