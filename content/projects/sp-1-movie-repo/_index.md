...
![TMDB logo](./images/blue-long.svg)
...
3. The JPA part should be implemented using Entities.
        - We use DTOs to convert the movie `json` from the TMDb API with Jackson.
        - We suggest that you create separate DTOs and Entities. The DTOs should be used to convert the `json` data from the TMDb API. The Entities should be used to store the data in the database.
        - Also, the Entities should be used as return types and arguments for the DAO methods. This means that DTOs should be used to transfer data between Main and the service layer. The DTOs are converted to Entities and vice versa in the service layer - and DAO methods are called with Entities.<br/><br/>![architecture](./images/architecture.svg)<br/><br/>
4. You should write DAO CRUD methods for the entities. The DAO methods should be implemented using JPA. You will also need to implement a service layer that uses the DAO methods and do the DTO to Entity conversion and vice versa there.
...
- Find a group to work with and [sign up on Moodle](https://cphbusiness.mrooms.net/mod/choicegroup/view.php?id=815729).
...
- Do some research on how to fetch the data from the TMDb API. You can use the [TMDB API reference](https://developer.themoviedb.org/reference/intro/getting-started) to get started.
...
- Hand in the [assignment on Moodle](https://cphbusiness.mrooms.net/mod/assign/view.php?id=815730). Make sure to include a link to the Github repository. One group member should hand in the assignment per group. NB! You can wait until Sunday evening to hand in the assignment.
