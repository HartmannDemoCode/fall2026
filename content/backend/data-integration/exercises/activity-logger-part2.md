...
This is the Friday exercise for Java Deep Dive II. The exercise is a continuation of the exercise from Tuesday. Your task is to persist the data from the external APIs in a database. You will also have the opportunity to practice using threads and executor service.

- The [first part of the exercise can be found here](../activity-logger-part1)
...
Unfortunately, the weather API is not very stable. Sometimes it returns json with missing values, but you should be able to handle that. Just ignore the missing values.
