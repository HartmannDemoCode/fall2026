...
2. Get an API key from TMDB. Use [this link to get an API key](https://www.themoviedb.org/settings/api). When you fill out the form you can be inspired by this: <br/><br/> ![API key form](../images/tmdb-api-form.png)
...
3. You will probably be familiar with IMDB for movies. <br/><br/> ![IMDB logo](../images/imdb-logo.svg)<br/><br/>
...
![env](../images/api-1.png)
![env](../images/api-2.png)
![env](../images/api-3.png)
...
Now the fun begins. We will use Java to fetch data from the `TMDB API`. We will use the [`HttpClient`]({{% ref "toolbox/dataintegration/httpclient" %}}) class to make the requests. We will also use the `ObjectMapper` class from the [Jackson library]({{% ref "toolbox/dataintegration/jackson" %}}) to [map]({{% ref "toolbox/dataintegration/dto-conversion" %}}) the [JSON]({{% ref "toolbox/dataintegration/json" %}}) response to [DTOs]({{% ref "toolbox/designpatterns/dto" %}}).
...
- Add methods to the `MovieService` that you find useful.
