...
- Java 17
- Maven
- Postgres 16.2 and PgAdmin running in Docker containers. You can follow [this tutorial](/toolbox/database/postgres-setup) if you don't have them set up.

## Exercise

1. Create a new JPA project. Use this [tutorial](/toolbox/java/orm/jpa-setup)
2. Create a new database called `points`
3. Update the `config.properties` file with the correct database name and password
...
12. Create a `DAO` class and transfer all methods that interact with the database to this class. Read more about the [DAO pattern here](/toolbox/java/orm/orm-wikipedia#4-data-access-object-dao-architecture).
13. Make each of the methods in the `DAO` class return a value instead of printing it to the console. Otherwise it's going to be difficult to test them. You do need to test the persistence of the data though.
    - The method that retrieves all Points for example should return a list of all the points from the database.
14. Add tests for each method in the `DAO` class. Use the `@BeforeAll` and `@AfterAll` annotations to set up and close the `EntityManagerFactory` and `EntityManager` objects.
