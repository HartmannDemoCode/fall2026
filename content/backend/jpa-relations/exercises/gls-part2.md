...
## Exercise: GLS Parcel Tracking System - Part 2

![gls delivery](../images/gls-shipments.png)

In this part of the exercise, we'll extend the GLS Parcel Tracking System to include more complex relationships between entities and introduce additional functionality.

When a parcel is sent from one location to another, it is considered a shipment. A shipment is a movement of a parcel between two locations. Each shipment is associated with a parcel, a source location, and a destination location. The parcel can have multiple shipments, but each shipment is associated with only one parcel, one source location, and one destination location. In the illustation above, a parcel is sent from Copenhagen to Aaarhus through four shipments between five locations.
...
1. Extend the existing parcel tracking system to include a new entity named "Location" to manage location information. The "Location" entity should have the following attributes:
...
2. Create a new entity named "Shipment" to represent the movement of parcels between locations. The "Shipment" entity should at least have the following attributes:

   - ID (auto-generated primary key)
   - Parcel (ManyToOne relationship with the Parcel entity)
...
3. Implement relationships between entities:
   - A parcel can have multiple shipments.
   - Each shipment is associated with one parcel, one source location, and one destination location.
