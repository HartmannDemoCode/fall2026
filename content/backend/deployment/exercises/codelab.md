...
First - read this overview carefully: [Full Pipeline](/toolbox/deployment-pipeline/full-pipeline)
...
- Follow the [Hotel API setup](/toolbox/deployment-pipeline/hotelAPI-setup) exercise to set up the hotelAPI on your Digital Ocean Droplet.
- Then follow the [Caddy Setup](/toolbox/deployment-pipeline/caddy-setup) exercise to set up Caddy and a reverse proxy for the hotel api on your Digital Ocean Droplet.

## Part 6: Setup Watchtower

Now that the hotelAPI is running on your Digital Ocean Droplet, we want to set up Watchtower to automatically deploy new versions of the hotelAPI when a new Docker Image is pushed to Docker Hub. Follow the [Watchtower Setup](/toolbox/deployment-pipeline/watchtower) exercise to add Watchtower to the `docker-compose.yml` file and complete the last step of the automated CI/CD pipeline.
